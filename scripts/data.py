...
def return_figures():
    ...
    layout_one = dict(title='Reward value of offer versus difficulty of redeeming offer',
    ...
    layout_two = dict(title='Offer attributes (Reward, Difficulty and Duration)',
                      xaxis=dict(title='Offer', ),
                      yaxis=dict(title='Value'),
                      )

    # Graph 3 - Starbucks app registrations (yearly)
    new_members = profile_pp['year_joined'].value_counts().rename_axis('year').reset_index(name='counts')
    ...
    layout_three = dict(title='Starbucks app new members per year',
    ...
    layout_four = dict(title='Age distribution of members',
